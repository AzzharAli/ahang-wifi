...
<table class="table table-light table-hover">
  ...
  <tbody>
    ...
    foreach($getDataInterface as $data){
        ...
        //Comment
        // if(!empty($data['comment'])){
        //     if(explode('-', $data['comment'], 3)[0] == "ALL"){
...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Secondary Group</h1>
      </div>
...
<table class="table table-light table-hover">
  ...
  <tbody>
    ...
    foreach($getDataInterface as $data){
        ...
        //Nama
        if(!empty($data['comment'])){
            if(explode('-', $data['comment'], 3)[0] == "SECONDARY"){
                echo "$td $nama</td>";
            }else{
...
var auto_refresh = setInterval(function(){$('#refresh').reload('traffic.php');}, 2000); // refresh every 10000 milliseconds
</script>
    <script>
  ...
  if($dataOn['status'] == "true"){
    ?><meta http-equiv="refresh" content="2;url=traffic.php"><?php
  }else{
